...
|---|---|---|---|---|
| OQ-25 | Oliver asked on 2026-09-16 for a `PLUGINS.md`, once the plugin work is complete, saying how to create a plugin and how to use one, written to guide a person and a future Claude session alike. `PLUGINS-GUIDE.md` already exists: 209 lines stating the contract, named as the authority on the binary interface by `ARCHITECTURE.md`, `DEVELOPMENT.md`, `README.md` and five Go files. Is `PLUGINS.md` that guide renamed and widened? Else is it a second document beside it? | Oliver | Before the document is written, once the plugin work is complete | Rename `PLUGINS-GUIDE.md` to `PLUGINS.md` and widen it, updating every reference, rather than writing a second document: two describing one contract would drift apart. It already holds the contract, where a plugin goes, cue ids, refusals and a checklist. What it lacks for the ask: using a plugin once built (putting it in the folder, casting its voice, what the log and the Cast pane say, keeping it on uninstall); a worked example built end to end with its toolchain named; how to test one against `plugintest`; a section written for an assistant, stating the invariants and the order to build in. |
