...
export const guideSections: GuideSection[] = [
  ...
  },
  {
    ...
    intro:
      "Choose here which moments are spoken for. Every moment the game raises is listed under its category, each with a switch: the knob sits at the right on orange while the moment is spoken for and at the left while it is switched off. A moment switched off stays quiet; the Status pane records it as off. The header above the list stays put while the list scrolls: in it a category's switch changes every moment in that category, while Switch all on and Switch all off change every moment, asking first wherever more than one would change. Each category's heading stays at the top of the list while its moments pass beneath it. Pressing a category's name in the header, marked with an arrow pointing down, moves the list to that category; pressing a heading, marked with an arrow showing whether it is open, hides its moments or shows them again. The switches belong to no voice, so casting another voice leaves them as they are; they are kept for the next start.",
  },
  {
