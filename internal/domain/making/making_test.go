...
func TestUnmadeGivesACuesLinesStillToMakeInLineOrder(t *testing.T) {
	all := making.New(voiced(t, "bə"), emma, files, noPauses, noEndings, nil).ToMake()
	plan := making.New(voiced(t, "bə"), emma, files, noPauses, noEndings, []string{all[1].Key})
...
func TestStaleGivesTheKeysOnDiskNoLineHolds(t *testing.T) {
	...
	got := making.New(voiced(t, "bə"), emma, files, noPauses, noEndings, onDisk).Stale()
...
func TestWithNothingMadeEveryLineIsToMakeInTheVoicesAccent(t *testing.T) {
	plan := making.New(voiced(t, "bə"), emma, files, noPauses, noEndings, nil)
	toMake := plan.ToMake()
	...
	if american := making.New(voiced(t, "bə"), michael, files, noPauses, noEndings, nil).ToMake()[0]; american.Sounds != "æə" {
...
func TestLinesWithAKeyOnDiskAreCurrentAndTheRestAreToMake(t *testing.T) {
	all := making.New(voiced(t, "bə"), emma, files, noPauses, noEndings, nil).ToMake()
	plan := making.New(voiced(t, "bə"), emma, files, noPauses, noEndings, []string{all[0].Key, all[2].Key, "no line's key"})
...
func TestALineWhoseSoundsChangedIsTheOnlyOneMadeAgain(t *testing.T) {
	made := keysOf(making.New(voiced(t, "bə"), emma, files, noPauses, noEndings, nil).ToMake())
	again := making.New(voiced(t, "bəz"), emma, files, noPauses, noEndings, made).ToMake()
...
func TestANewStyleFileMakesEveryLineAgain(t *testing.T) {
	made := keysOf(making.New(voiced(t, "bə"), emma, files, noPauses, noEndings, nil).ToMake())
	plan := making.New(voiced(t, "bə"), emma, making.Files{Style: "style-2", Model: files.Model}, noPauses, noEndings, made)
...
func TestACuesTakesAreTheDistinctKeysOfItsCurrentLines(t *testing.T) {
	all := making.New(voiced(t, "bi"), emma, files, noPauses, noEndings, nil).ToMake()
	plan := making.New(voiced(t, "bi"), emma, files, noPauses, noEndings, []string{all[2].Key, all[1].Key})
