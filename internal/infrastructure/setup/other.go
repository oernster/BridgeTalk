...
// ErrUnsupported says this platform has no bespoke setup program. Linux and macOS
// are served by their own packaging rather than by a per-user installer, so the
// stubs here exist to keep the package building and vetting everywhere, not to offer
// a second install route.
var ErrUnsupported = errors.New("the setup program runs on Windows only")

// ErrAppRunning matches the Windows sentinel so the setup facade compiles anywhere.
var ErrAppRunning = ErrUnsupported

// ErrAppStillRunning matches the Windows sentinel so the setup facade compiles anywhere.
